    ...
    try {
      ...
      for (var i = 0; i < list.length; i++) {
        fetch(URL_GETSTAFF, {
          ...
          throw new Error('convertData getStaff response was not ok.');
        }).then(result => {
          const data = {
            "id": list[i].id,
            "name": result.name,
  ...
  render() {
    ...
    return __jsx("div", null, __jsx(next_head__WEBPACK_IMPORTED_MODULE_2___default.a, null, __jsx("script", {
      ...
      SubComponent: row => {
        ...
        }, " \u4EE3\u7406\u4EBA\uFF1A", row.original.substitute, "\xA0\xA0\xA0\u62D2\u7D55\u539F\u56E0\uFF1A", row.original.rejectReason, " ") : __jsx("div", {
          style: {
            display: "table-cell",
            "vertical-align": "middle"
          }
        }, " \u4EE3\u7406\u4EBA\uFF1A", row.original.substitute, " ")), __jsx("div", {
          style: {
            display: "inline-block",
            float: "right"
          }
