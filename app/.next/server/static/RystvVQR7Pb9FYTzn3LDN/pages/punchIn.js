...
class App extends react__WEBPACK_IMPORTED_MODULE_1__["Component"] {
  ...
  getPunch() {
    try {
      const URL_GETPUNCH = `https://us-central1-teddybear-dev.cloudfunctions.net/api/punch/get?ubn=${this.state.ubn}&query=email&value=${this.state.email}`;
  ...
  render() {
    ...
    }, __jsx(BOX, null, this.state.email & this.state.punch ? __jsx(BUTTON, {
      fontFamily: "Candara",
      variant: "contained",
      color: "secondary",
      disabled: (!this.state.latitude | !this.state.longitude) & this.state.punch.length == 0,
      onClick: () => {
        this.createPunch();
      }
